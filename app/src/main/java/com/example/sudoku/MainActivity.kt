...
class MainActivity : ComponentActivity() {
    ...
    private fun initializeVars() {
        sudokuBoard = generatePuzzle("Easy")
    }

    // UI Initialization
    private fun initializeUI() {
        sudokuGrid = findViewById(R.id.sudokuGrid)
        initializeGrid(sudokuGrid, sudokuBoard)
    }

    private fun initializeGrid(gridLayout: GridLayout, board: Array<IntArray>) {
    ...
    // Game Logic
    private fun generatePuzzle(difficulty: String): Array<IntArray> {
        ...
        if (fillBoard(grid)) {
            ...
            if (validateBoard(grid)) {
                ...
                if (validateBoard(grid)) {
                    ...
                    Toast.makeText(this, "Puzzle generation failed", Toast.LENGTH_SHORT).show()
                    finish()
                }
            } else {
                Log.e("generatePuzzle", "Full board failed validation check")
                Toast.makeText(this, "Board generation failed", Toast.LENGTH_SHORT).show()
                finish()
            }
        }
        else {
            Log.e("generatePuzzle", "Failed to fill board")
            Toast.makeText(this, "Failed to fill board", Toast.LENGTH_SHORT).show()
    ...
    private fun isUnique(array: IntArray): Boolean {
    ...
    private fun getColumn(board: Array<IntArray>, col: Int): IntArray {
        return IntArray(board.size) { row -> board[row][col] }
    }

    // Helper to get a sub-grid as an array
    private fun getSubGrid(board: Array<IntArray>, startRow: Int, startCol: Int, size: Int): IntArray {
